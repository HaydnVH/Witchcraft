...
namespace hvh {
  ...
  public:
    ...
    friend inline void swap(_soa_base<Ts...>& lhs, _soa_base<Ts...>& rhs) {
      std::swap(lhs.mysize, rhs.mysize);
      std::swap(lhs.mycapacity, rhs.mycapacity);
    }
  ...
  public:
    ...
    typename std::enable_if<
        K != 0, typename elem_type_holder<K, _soa_base<FT, RTs...>>::
    ...
    template <size_t K>
    typename std::enable_if<
        K != 0, const typename elem_type_holder<K, _soa_base<FT, RTs...>>::
    ...
    template <size_t K>
    typename std::enable_if<
        K != 0, typename elem_type_holder<K, _soa_base<FT, RTs...>>::type&>::
    ...
    // Does not perform bounds checking; do not use with an out-of-bounds index!
    template <size_t K>
    typename std::enable_if<K != 0, const typename elem_type_holder<
                                        K, _soa_base<FT, RTs...>>::type&>::
    ...
    // equals 'goal'. Otherwise, returns the number of items which are less than
    // 'goal'. This behaviour is similar to std::lower_bound. Complexity:
    // O(logn).
    template <size_t K>
    typename std::enable_if<K != 0, size_t>::type inline lower_bound(
        const typename elem_type_holder<K, _soa_base<FT, RTs...>>::type& goal) {
      _soa_base<RTs...>& base = *this;
      return base.template lower_bound<K - 1>(goal);
    }
    // lower_bound_row<K>(goal_row)
    // Performs a binary search looking for the Kth entry in 'goal_row' in
    // sorted array 'K'. This behaves like lower_bound, but letting the user
    ...
    // other than the Kth are ignored. Complexity: O(logn).
    template <size_t K>
    typename std::enable_if<K != 0, size_t>::type inline lower_bound_row(
        const FT& first, const RTs&... rest) {
      _soa_base<RTs...>& base = *this;
      return base.template lower_bound<K - 1>(rest...);
    }
    ...
    // std::upper_bound, and is useful if one array in a container is treated as
    // the key for a binary search table. Complexity: O(logn).
    template <size_t K>
    typename std::enable_if<K == 0, size_t>::type
    ...
    // the key for a binary search table. Complexity: O(logn).
    template <size_t K>
    typename std::enable_if<K != 0, size_t>::type inline upper_bound(
        const typename elem_type_holder<K, _soa_base<FT, RTs...>>::type& goal) {
      _soa_base<RTs...>& base = *this;
      return base.template upper_bound<K - 1>(goal);
    }
    ...
    inline void construct_range(size_t begin, size_t end) {
      for (size_t i = begin; i < end; ++i) { new (&mydata[i]) FT(); }
      _soa_base<RTs...>& base = *this;
      base.construct_range(begin, end);
    }

    // construct_range calls the copy constructor on a range of entries.
    inline void construct_range(size_t begin, size_t end, const FT& initval,
                                const RTs&... restvals) {
      for (size_t i = begin; i < end; ++i) { new (&mydata[i]) FT(initval); }
      _soa_base<RTs...>& base = *this;
    ...
    inline void destruct_range(size_t begin, size_t end) {
      for (size_t i = begin; i < end; ++i) { mydata[i].~FT(); }
      _soa_base<RTs...>& base = *this;
    ...
    inline void divy_buffer(void* newmem) {
      if (mydata) {
        memcpy(newmem, mydata,
               sizeof(FT) * std::min(this->mysize, this->mycapacity));
      }
      mydata                  = (FT*)newmem;
      _soa_base<RTs...>& base = *this;
    ...
    typename std::enable_if<std::is_copy_constructible<FirstType>::value,
                            void>::type inline push_back(const FirstType& first,
                                                         RestTypes&&... rest) {
    ...
    typename std::enable_if<std::is_move_constructible<FirstType>::value,
                            void>::type inline push_back(FirstType&& first,
                                                         RestTypes&&... rest) {
    ...
    typename std::enable_if<
        ...
        void>::type inline emplace_back(const FirstType& first,
                                        RestTypes&&... rest) {
      new (&mydata[this->mysize]) FT(first);
      _soa_base<RTs...>& base = *this;
    ...
    typename std::enable_if<
        ...
        void>::type inline emplace_back(FirstType&& first,
                                        RestTypes&&... rest) {
      new (&mydata[this->mysize]) FT(std::move(first));
      _soa_base<RTs...>& base = *this;
      base.emplace_back(rest...);
    ...
    inline void emplace_back(const std::tuple<FirstTypes...>& first,
                             const RestTypes&... rest) {
      std::apply(
          [this](const FirstTypes&... args) {
            new (&mydata[this->mysize]) FT(args...);
          },
          first);
      _soa_base<RTs...>& base = *this;
      base.emplace_back(rest...);
    ...
    // inserts (copies) a row at the specified index, moving later entries back
    // by one.
    template <typename FirstType = FT, typename... RestTypes>
    typename std::enable_if<std::is_copy_constructible<FirstType>::value,
                            void>::type inline insert(size_t           location,
                                                      ...
                                                      RestTypes&&... rest) {
      memmove(mydata + (location + 1), mydata + location,
              sizeof(FT) * (this->mysize - location));
    ...
    template <typename... FirstTypes, typename... RestTypes>
    inline void emplace(size_t location, const std::tuple<FirstTypes...>& first,
                        RestTypes&&... rest) {
      memmove(mydata + (location + 1), mydata + location,
              sizeof(FT) * (this->mysize - location));
      std::apply(
          [this, location](const FirstTypes&... args) {
            new (&mydata[location]) FT(args...);
          },
          first);
      _soa_base<RTs...>& base = *this;
      base.emplace_back(location, rest...);
    ...
    inline void erase_shift(size_t location) {
      mydata[location].~FT();
      memmove(mydata + location, mydata + (location + 1),
              sizeof(FT) * (this->mysize - location));
      _soa_base<RTs...>& base = *this;
    ...
    friend inline void swap(_soa_base<FT, RTs...>& lhs,
                            _soa_base<FT, RTs...>& rhs) {
      using std::swap;
    ...
    inline std::tuple<FT&, RTs&...> make_row_tuple(size_t row) {
      ...
      return std::tuple_cat(
          std::make_tuple(std::reference_wrapper<FT>(mydata[row])),
          base.make_row_tuple(row));
    }

    // Creates tuple of const references representing a whole row.
    inline std::tuple<const FT&, const RTs&...>
        make_row_tuple(size_t row) const {
      const _soa_base<RTs...>& base = *this;
      return std::tuple_cat(
          std::make_tuple(std::reference_wrapper<FT>(mydata[row])),
          base.make_row_tuple(row));
  ...
  public:
    ...
    soa(const std::initializer_list<std::tuple<Ts...>>& initlist) {
      ...
      for (auto& entry : initlist) {
        std::apply([this](const Ts&... args) { this->push_back(args...); },
                   entry);
      }
    }
    // soa(&& rhs)
    ...
    bool reserve(size_t newsize) {
      ...
      // Allocate new memory.
      void* alloc_result =
          _soa_aligned_malloc(16, base.size_per_entry() * newsize);
    ...
    void*  get_raw_data() { return this->template data<0>(); }
    ...
    struct const_iterator {
      ...
      const_iterator(const soa& container, size_t row):
          _container(container), _row(row) {}
