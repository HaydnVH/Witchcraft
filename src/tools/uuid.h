...
namespace wc {
  ...
  struct Uuid {
    ...
    /// Generates a Max UUID (all 1s).
    inline constexpr static Uuid makeMax() { return {UINT64_MAX, UINT64_MAX}; }
    ...
    static Uuid makeV4() {
      ...
      // Ideally, it should use a 128-bit RNG (maybe?)
    ...
    /// Merges multiple UUIDs together.
    /// This operation is order-independant, unlike using UUIDs to generate a V5 UUID.
    /// The result is a "V0" UUID because it doesn't follow any existing specs.
    template <typename It>
    constexpr static Uuid merge(It begin, It end) {
      Uuid result {};
      for (It it {begin}; it != end; ++it) {
        result.lo_ ^= it->lo_;
        result.hi_ ^= it->hi_;
      }
      result.setVersionBits(0);
      return result;
    }
    ...
    /// Merges two ids together.  This operation is order-independent.
    /// The result is a "V0" UUID because it doesn't follow any existing specs.
    inline constexpr friend Uuid operator^(Uuid lhs, Uuid rhs) noexcept {
      Uuid result {lhs.lo_ ^ rhs.lo_, lhs.hi_ ^ rhs.hi_};
      result.setVersionBits(0);
      return result;
    }
    ...
    inline constexpr Uuid operator~() const noexcept {
      // Since the "version" halfbyte is always either 0, 4, or 5,
    ...
    inline constexpr bool isNot() const noexcept {
      // In a proper (non-Not) id, the version halfbyte is either 0 (0000), 4 (0100),
      // or 5 (0101). If the id has been bit-flipped, that halfbyte will be either
      // F (1111), B (1011), or A (1010). We'll use the 4th bit of the version halfbyte,
    ...
    /// Overloads operator << so UUIDs can be sent to an ostream.
    friend std::ostream& operator<<(std::ostream& os, const Uuid& self) {
      return os << self.toStrCanon();
    }

  private:
    /// Sets the bits that make a UUID a UUID and not just a pile of random data.
