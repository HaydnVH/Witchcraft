...
bool wc::Archive::open(const char* archiveName) {
  ...
  if (!file_.is_open()) {
    dbg::error(std::format("Archive '{}' could not be opened.", archiveName));
    return false;
  }
  ...
  if (strncmp(header_.magic, Archive::MAGIC, 8) != 0) {
    dbg::error(
        std::format("Archive '{}' is not a valid archive.", archiveName));
    file_.close();
    return false;
  }
  ...
  if (header_.numfiles > 0) {
    ...
    if (bytesread != dictbytes) {
      dbg::error(
          std::format("Failed to load archive dictionary data.\n"
                      "Expected {} bytes, read {}.",
                      dictbytes, bytesread));
...
void wc::Archive::close() {
  ...
  if (modified_) {
    ...
    if (filesDeleted_) {
      ...
      //               std::format("Expected {} bytes, wrote {}.", dictbytes,
      //                           byteswritten)});
      // }
    }
  }
...
bool wc::Archive::extractData(const char* path, std::vector<char>& buffer,
  ...
  if (index == SIZE_MAX) {
    dbg::errmore(std::format("Failed to find file '{}'.", path));
    return false;
  }
  ...
  if (file_.eof()) {
    dbg::errmore(std::format("Bad offset for file '{}'.", path));
    return false;
  }
  ...
  // so the file info doesn't need to know which was used for compression.
    ...
    if (result < 0) {
      dbg::error(
          std::format("Failed to decompress data for '{}',\n"
...
void wc::Archive::extractFile(const char* path, const char* dstfilename) {
  ...
  if (ec) {
    dbg::error(
        std::format("Error extracting file '{}',\n"
                    "Filesystem error: {}.",
                    path, ec.message()));
    return;
  }
...
bool wc::Archive::insertData(const char* path, void* buffer, int32_t size,
  ...
  if (compress != DO_NOT_COMPRESS) {
    ...
    if (newinfo.sizeCompressed <= 0) {
      dbg::warning(
          std::format("Failed to compress file '{}',\n"
...
void wc::Archive::pack(const char* srcfolder, ReplaceEnum replace,
  ...
  if (!std::filesystem::exists(srcpath)) {
    dbg::error(std::format("'{}' does not exist.", srcfolder));
    return;
  }
  if (!std::filesystem::is_directory(srcpath)) {
    dbg::error(std::format("'{}' is not a directory.", srcfolder));
    return;
  }
...
void wc::Archive::unpack(const char* dstfolder) {
  ...
  if (!std::filesystem::is_directory(dstpath)) {
    dbg::error(
        std::format("'{}' already exists and is not a directory.", dstfolder));
    return;
  }
